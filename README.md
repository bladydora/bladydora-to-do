...
- 全局待办同步：由 9004 从 `../全局待办清单.md` 预览导入，先过滤已忽略/过期项并识别重复项，再由人类确认导入。
...
- Dispatcher 只写日志化、可审计的待办结论，例如 `../全局待办清单.md`；它不直接写 App 运行数据库。
- 9004-行动清单-master-agent / To Do App 拥有同步权，负责去重、过期项过滤、状态映射和人工确认。
